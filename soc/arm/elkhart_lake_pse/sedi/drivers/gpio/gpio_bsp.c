...
#define GPIO_SET_BIT(base, reg, index, bit)                                    \
	((base->reg[index]) |= SET_MASK(bit))
#define GPIO_CLEAR_BIT(base, reg, index, bit)                                  \
	((base->reg[index]) &= CLEAR_MASK(bit))
...
static int32_t gpio_set_power(IN sedi_gpio_t gpio_device,
...
static void gpio_write_pin_port(IN sedi_gpio_t gpio_device,
				IN gpio_port_access_t access_type,
				IN uint8_t group,
				IN sedi_gpio_pin_state_t state)
{
...
static uint32_t gpio_read_pin_port(IN sedi_gpio_t gpio_device,
				   IN gpio_port_access_t access_type,
				   IN uint8_t group)
{
	uint8_t port = (group >> access_type);
...
static void gpio_isr(IN sedi_gpio_t gpio_device)
{
...
SEDI_ISR_DECLARE(sedi_gpio_1_isr)
{
	gpio_isr(SEDI_GPIO_1);
}
...
int32_t sedi_gpio_get_pin_function(IN sedi_gpio_t gpio_device, IN uint32_t pin)
{
	uint32_t reg_addr, data = 0;
	sb_port_t port = SB_GPIO_COMM0;

	/* Get the pin function value */
	switch (gpio_device) {
	case SEDI_GPIO_0:
		port = SB_GPIO_COMM0;
		break;
	...
	reg_addr = GPIO_PAD_REG_START + pin * GPIO_PAD_REG_OFFSET;
	sedi_sideband_send(SEDI_SIDEBAND_0, port, SEDI_SIDEBAND_ACTION_READ,
			   reg_addr, 0);
	sedi_sideband_wait_ack(SEDI_SIDEBAND_0, port, SEDI_SIDEBAND_ACTION_READ,
			       &data);

	return ((data & GPIO_PAD_FUNCTION_MASK) >> GPIO_PAD_FUNCTION_SHIFT);
}
			  ...
			  IN sedi_gpio_pin_config_t pin_config)
{
	...
	if (GPIO_DIR_MODE_OUTPUT == pin_config.direction) {
		...
		if (pin_config.interrupt_mode & GPIO_INT_MODE_FALLING_EDGE) {
			GPIO_SET_BIT(base, gfer, port, offset);
		}

		if (pin_config.interrupt_mode & GPIO_INT_MODE_RISING_EDGE) {
...
void sedi_gpio_write_pin_8bits(IN sedi_gpio_t gpio_device, IN uint8_t group,
			       IN sedi_gpio_pin_state_t pin_state)
{
	gpio_write_pin_port(gpio_device, SEDI_GPIO_8BITS_ACCESS, group,
			    pin_state);
}
...
void sedi_gpio_write_pin_16bits(IN sedi_gpio_t gpio_device, IN uint8_t group,
				IN sedi_gpio_pin_state_t pin_state)
{
	gpio_write_pin_port(gpio_device, SEDI_GPIO_16BITS_ACCESS, group,
			    pin_state);
}
...
void sedi_gpio_write_pin_32bits(IN sedi_gpio_t gpio_device, IN uint8_t group,
				IN sedi_gpio_pin_state_t pin_state)
{
	gpio_write_pin_port(gpio_device, SEDI_GPIO_32BITS_ACCESS, group,
			    pin_state);
}
					 ...
					 IN uint32_t pin)
{
	...
	return pin_val ? GPIO_STATE_HIGH : GPIO_STATE_LOW;
}
