...
/*!
 * \def PM_VNN_ALL_RESET
 * \brief Macro for reset all VNNREQ.
 * \ingroup driver_pm_internal
 */
#define PM_VNN_ALL_RESET()                                                     \
	write32(PMU_VNN_REQ_31_0, read32(PMU_VNN_REQ_31_0));                   \
	write32(PMU_VNN_REQ_63_32, read32(PMU_VNN_REQ_63_32));                 \
	write32(PMU_VNN_REQ_ACK, read32(PMU_VNN_REQ_ACK));

/*!
 * \def PM_IS_IN_SX
 * \brief Macro to check if Host is in Sx.
 * \ingroup driver_pm_internal
 */
#define PM_IS_IN_SX()                                                \
	((read32(PMU_RESET_PREP) & PMU_RESET_PREP_AVAIL) ||          \
	    !(read32(PMU_HOST_RSTB_INTR) & PMU_HOST_RST_B_LIVESTS))
...
/*!
 * \fn void pm_return_from_idle(IN int32_t ticks)
 * \brief The hook function when exit idle state
 * \return void
 */
void pm_return_from_idle(IN int32_t ticks);
