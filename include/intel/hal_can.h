...
#ifndef _INTEL_DRIVER_CAN_H_
#define _INTEL_DRIVER_CAN_H_
...
#define INTEL_CAN_INSTANCE(n)                                                  \
	((intel_instance_t *)((intel_can_context[]) {                    \
	    {.id = CAN_##n,                                                    \
	    }}))
 ...
 */
static inline uint8_t intel_can_msg_len_to_dlc(IN uint32_t len)
{
 ...
 */
static inline uint8_t intel_can_dlc_to_msg_len(IN uint32_t dlc)
{
...
typedef void (*can_callback_t)(IN intel_instance_t *inst, IN enum can_id id, IN uint32_t val,
			       IN void *data);
 ...
 */
int32_t intel_can_init(IN intel_instance_t *inst, IN int inst_num, IN can_callback_t cb,
...
int32_t intel_can_send_message(IN intel_instance_t *inst,
			       IN enum can_msg_id msgID, IN void *frame);
...
int32_t intel_can_set_power(IN intel_instance_t *inst,
...
#include <intel/hal_can_priv.h>
#endif /* #ifndef INTEL_DRIVER_CAN_H_ */
