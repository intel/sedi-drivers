...
static const dma_resources_t resources[SEDI_DMA_NUM] = {
	...
	{.regs = (dma_ann_1p0_regs_t *)SEDI_DMA_1_REG_BASE},
	{.regs = (dma_ann_1p0_regs_t *)SEDI_DMA_2_REG_BASE},
};
			      ...
			      INOUT sedi_dma_capabilities_t *cap)
{
	...
	if (DEV_PSE_OWNED ==
	    sedi_get_dev_ownership(PSE_DEV_DMA0 + dma_device)) {
...
static void config_snoop(sedi_dma_t dma_device, int channel_id)
{
	...
	volatile pse_dma_misc_regs_t *misc_regs = &(regs->pse_misc_reg);
	/* snoop */
			   ...
			   IN sedi_power_state_t state)
{
	...
	driver_id_t dma_pm_id = dma_device + DRIVER_ID_DMA0;

	switch (state) {
	case SEDI_POWER_FULL:
		if (dma_context[dma_device].power_status == 0) {
			pm_driver_start_trans(dma_pm_id);
		}
...
static int32_t dma_apply_other_regs(volatile pse_dma_misc_regs_t *misc_regs,
...
SEDI_ISR_DECLARE(sedi_dma_0_isr)
{
	dma_isr(SEDI_DMA_0);
}

SEDI_ISR_DECLARE(sedi_dma_1_isr)
{
	dma_isr(SEDI_DMA_1);
}

SEDI_ISR_DECLARE(sedi_dma_2_isr)
{
	dma_isr(SEDI_DMA_2);
}
