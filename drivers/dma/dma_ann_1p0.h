...
typedef struct {
	...
	__IO_R uint32_t reserved0[(0x3b8 - 0x3a8) >> 2];
	...
	__IO_R uint32_t reserved1[(0x400 - 0x3c8) >> 2];
...
typedef struct {
	...
	__IO_RW uint32_t dma_regaccess_chid; /**<DMA_REGACCESS_CHID >*/
	__IO_RW uint32_t dma_ecc_err_sresp;  /**<DMA_ECC_ERR_SRESP >*/
	__IO_RW uint32_t d0i3c;		     /**<D0I3C>*/
} pse_dma_misc_regs_t;

/*dma registers address map structure */
typedef struct {
	__IO_RW dma_chan_reg_t chan_reg[DMA_CHANNEL_NUM]; /**< Channel Reg */
	__IO_RW dma_int_reg_t int_reg; /**< Interrupt Register */
	__IO_R uint32_t reserved0[(0x398 - 0x368) >> 2];
	__IO_RW dma_chann_misc_reg_t misc_reg; /**< Miscellaneous Register */
	__IO_R uint32_t reserved1[(0x1000 - 0x420) >> 2];
	__IO_RW pse_dma_misc_regs_t pse_misc_reg;
} dma_ann_1p0_regs_t;
