...
{
	wait_for_idle(GENERAL_CONFIG | MAIN_COUNTER_VALUE | TIMER0_CONFIG_CAPS |
		      TIMER0_COMPARATOR | TIMER1_CONFIG_CAPS |
		      TIMER1_COMPARATOR | TIMER2_CONFIG_CAPS |
		      TIMER2_COMPARATOR);
	/*
...
void hpet_timer_isr(IN sedi_hpet_t timer_id)
{
	...
	if (bsp_timers[timer_id].one_shot) {
		...
		bsp_timers[timer_id].expires =
		    curtick + bsp_timers[timer_id].timeout;
		/* Set new comparator */
...
int32_t sedi_hpet_start_timer(IN sedi_hpet_t timer_id)
{
	...
	bsp_timers[timer_id].expires =
	    CURRENT_HPET_CYCLE + bsp_timers[timer_id].timeout;
