...
static void init_i2c_prescale(void)
{
	if (regval_scl_hcnt[I2C_SPEED_STANDARD] == 0) {
		regval_scl_hcnt[I2C_SPEED_STANDARD] =
		    LBW_CLK_MHZ * I2C_SS_SCL_HIGH / NS_PER_US;
		regval_scl_hcnt[I2C_SPEED_FAST] =
		    LBW_CLK_MHZ * I2C_FS_SCL_HIGH / NS_PER_US;
		regval_scl_hcnt[I2C_SPEED_FAST_PLUS] =
		    LBW_CLK_MHZ * I2C_FSP_SCL_HIGH / NS_PER_US;
		regval_scl_hcnt[I2C_SPEED_HIGH] =
		    LBW_CLK_MHZ * I2C_HS_SCL_HIGH / NS_PER_US;

		regval_scl_lcnt[I2C_SPEED_STANDARD] =
		    LBW_CLK_MHZ * I2C_SS_SCL_LOW / NS_PER_US;
		regval_scl_lcnt[I2C_SPEED_FAST] =
		    LBW_CLK_MHZ * I2C_FS_SCL_LOW / NS_PER_US;
		regval_scl_lcnt[I2C_SPEED_FAST_PLUS] =
		    LBW_CLK_MHZ * I2C_FSP_SCL_LOW / NS_PER_US;
		regval_scl_lcnt[I2C_SPEED_HIGH] =
		    LBW_CLK_MHZ * I2C_HS_SCL_LOW / NS_PER_US;
	}
}
...
static int dw_i2c_poll_write(uint32_t base, const uint8_t *buffer,
			     uint32_t length, bool pending)
{
	i2c_dw_apb200a_regs_t *i2c = (i2c_dw_apb200a_regs_t *)base;
...
static int dw_i2c_poll_read(uint32_t base, uint8_t *buffer, uint32_t length,
			    bool pending)
{
	i2c_dw_apb200a_regs_t *i2c = (i2c_dw_apb200a_regs_t *)base;
...
static void dw_i2c_abort(uint32_t base)
{
	i2c_dw_apb200a_regs_t *i2c = (void *)base;
	uint32_t value;
...
static const sedi_driver_version_t driver_version = {SEDI_I2C_API_VERSION,
						     SEDI_I2C_DRV_VERSION};
...
#define I2C_CONTEXT_INIT(x)                                                    \
	{                                                                      \
		.base = SEDI_I2C_##x##_REG_BASE,                               \
		.dma_handshake = DMA_HWID_I2C##x, .speed = I2C_SPEED_FAST,     \
			      ...
			      sedi_i2c_capabilities_t *cap)
{
	...
	if (DEV_PSE_OWNED ==
	    sedi_get_dev_ownership(PSE_DEV_I2C0 + i2c_device)) {
...
int32_t sedi_i2c_init(IN sedi_i2c_t i2c_device, IN sedi_i2c_event_cb_t cb_event)
{
	DBG_CHECK(i2c_device < SEDI_I2C_NUM, SEDI_DRIVER_ERROR_PARAMETER);
			   ...
			   IN sedi_power_state_t state)
{
	...
	struct i2c_context *context = &contexts[i2c_device];
	driver_id_t id = i2c_pm_id[i2c_device];
...
static void callback_tx_dma_transfer(const sedi_dma_t dma, const int chan,
...
static void callback_rx_dma_transfer(const sedi_dma_t dma, const int chan,
				     const int event, void *param)
{
	struct i2c_context *context = &contexts[(int)param];
	i2c_dw_apb200a_regs_t *i2c = (i2c_dw_apb200a_regs_t *)(context->base);
...
static void callback_rx_cmd_dma_transfer(const sedi_dma_t dma, const int chan,
					 const int event, void *param)
{
	struct i2c_context *context = &contexts[(int)param];
	i2c_dw_apb200a_regs_t *i2c = (i2c_dw_apb200a_regs_t *)(context->base);
...
static int config_and_enable_dma_channel(sedi_i2c_t i2c_dev, int dma,
					 int handshake, int chan, uint32_t src,
				  ...
				  IN bool pending)
{
	...
	config_and_enable_dma_channel(
	    i2c_device, dma, context->dma_handshake, dma_chan, (uint32_t)data,
	    dw_i2c_dr_address(context->base), size, I2C_DMA_DIRECTION_TX);
				 ...
				 IN uint32_t num, IN bool pending)
{
	...
	config_and_enable_dma_channel(i2c_device, dma, context->dma_handshake,
				      cmd_dma_chan, (uint32_t)&dma_cmd,
				      dw_i2c_dr_address(context->base) + 1,
				      num - 2, I2C_DMA_DIRECTION_RX_CMD);

	/* data transfer */
	config_and_enable_dma_channel(
	    i2c_device, dma, context->dma_handshake, dma_chan,
...
SEDI_ISR_DECLARE(sedi_i2c_0_isr)
{
	dw_i2c_isr(SEDI_I2C_0);
}

SEDI_ISR_DECLARE(sedi_i2c_1_isr)
{
	dw_i2c_isr(SEDI_I2C_1);
}

SEDI_ISR_DECLARE(sedi_i2c_2_isr)
{
	dw_i2c_isr(SEDI_I2C_2);
}

SEDI_ISR_DECLARE(sedi_i2c_3_isr)
{
	dw_i2c_isr(SEDI_I2C_3);
}

SEDI_ISR_DECLARE(sedi_i2c_4_isr)
{
	dw_i2c_isr(SEDI_I2C_4);
}

SEDI_ISR_DECLARE(sedi_i2c_5_isr)
{
	dw_i2c_isr(SEDI_I2C_5);
}

SEDI_ISR_DECLARE(sedi_i2c_6_isr)
{
	dw_i2c_isr(SEDI_I2C_6);
}

SEDI_ISR_DECLARE(sedi_i2c_7_isr)
{
	dw_i2c_isr(SEDI_I2C_7);
}
